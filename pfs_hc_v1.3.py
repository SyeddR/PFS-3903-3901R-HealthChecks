#!/usr/bin/python
### version 1.3:-
### Adding blade serial number
### version 1.2:-
### separating 3903 and 3901 data
### version 1.1 :-
...
TACAS          = True
FOLDER         = '/opt/PFS3903_HC' ### Set the path to script's folder
...
class Server:
   ...
   # _Status       = None
    ...
    def __init__(self,ip,username,password,timeout,timeinterval,alias=None):
        ...
        self._switches3903data = None
        self._switches3901Rdata =None
    ...
    def AddSwitches_3901Rdata(self, data):
        self._switches3901Rdata = data

    def AddSwitches_3903data(self, data):
        self._switches3903data = data
...
def Scan1(server):
  ...
  try:
        ...
        i = p.expect(["ERROR:", pexpect.TIMEOUT,pexpect.EOF, COMMAND_PROMPT], timeout=500)
        if i == 0:
            
            print "Server returned an error:"+p.after
        elif i == 1:

            print "Failed to get server during 'sho sys ala curr': Timeout"
        elif i == 2:

            print "    Lost connection to the server during 'sho sys ala curr" 
        elif i == 3: 
        ...
        if i == 0:
            ...
            print "Failed to get server during 'sho port ala curr': Timeout"
        elif i == 2:
            
            print "    Lost connection to the server during 'sho port ala curr'" 
        elif i == 3: 
...
def Getbladestatus(p,blade):
  try :
        ...
        #p.expect(COMMAND_PROMPT)
           ...
           blade._serial=re.findall('Serial Number:\s*(.*)',data)[0].rstrip()
...
def Log_Results(server):
    try:
       ...
       server._switches3903data = [row for row in switches_data if row[4]=='3903']
       server._switches3901Rdata = [row for row in switches_data if row[4]=='3901R']
...
def proto_main(argv):
    ...
    file2.write('********3903 PFS******\n')
    file2.write(','.join(switch3903_hdr))
    file2.write('\n')
    ...
    port_curr_alarm=''
    sys_curr_alarm=''
    ## variable to check if there is 3901R PFS
    PFS3901R=False
    ...
    for server in servers: 
        ...
        ## Set PFS3901R to true if server has 3901R data
        if server._switches3901Rdata is not None:
          PFS3901R=True
        
        #### Writing 3903 data in first loop        
        if server._switches3903data is not None:
          
          print server._switches3903data
          for data in server._switches3903data:
           file2.write(','.join(data))
           file2.write('\n')
           
           na_data=[]
           if len(data) < len(switch3903_hdr):
               na_data=['NA' for i in range(len(switch3903_hdr)-len(data))]  
               
           switches_3903data.append(data+na_data)   
        ###############Port scan data
        if Last_Scan_time:
    ...
    ### running Second loop to write 3901R data
    if PFS3901R:
        file2.write('\n\n')
        file2.write('********3901R PFS******\n')
        file2.write(','.join(switch3901R_hdr))
        file2.write('\n')
        for server in servers:
            if server._switches3901Rdata is not None:
              
              print server._switches3901Rdata
              for data in server._switches3901Rdata:
               file2.write(','.join(data))
               file2.write('\n')
               ...
               switches_3901Rdata.append(data+na_data)   
        
    print switches_3901Rdata
    print switches_3903data    
    file1.close() 
    file2.close()
    ...
    switch_3903err_dic={
      'Switch Status':['^(?!Active)',switch3903_hdr.index('Switch Status')],
      'PSU1 Error':['^(?!(?:GOOD|NA))',switch3903_hdr.index('PSU1 Status')],
      'PSU2 Error':['^(?!(?:GOOD|NA))',switch3903_hdr.index('PSU2 Status')],
      'FAN Error':['^(?!(?:GOOD|NA))',switch3903_hdr.index('FAN Status')],
      #'Max Power Budget',
      #'Slot 1 Status',
      #'Slot 2 Status',
      #'Slot 3 Status',
      #'Active Controller',
      'Blade 1.01 Error':['^(?!(?:GOOD|NA))',switch3903_hdr.index('Blade 1.01 Status')],
      'Blade 1.02 Error':['^(?!(?:GOOD|NA))',switch3903_hdr.index('Blade 1.02 Status')],
      'Blade 1.03 Error':['^(?!(?:GOOD|NA))',switch3903_hdr.index('Blade 1.03 Status')],
      'Blade 1.01 Temp':['^[5-9]* C',switch3903_hdr.index('Blade 1.01 Temp')],
      'Blade 1.02 Temp':['^[5-9]* C',switch3903_hdr.index('Blade 1.02 Temp')],
      'Blade 1.03 Temp':['^[5-9]* C',switch3903_hdr.index('Blade 1.03 Temp')],
      'Blade 1.01 Conn Filter > 90%':['^9[1-9]|^100',switch3903_hdr.index('Blade 1.01 Temp')],
      'Blade 1.02 Conn Filter > 90%':['^9[1-9]|^100',switch3903_hdr.index('Blade 1.02 Temp')],
      'Blade 1.03 Conn Filter > 90%':['^9[1-9]|^100',switch3903_hdr.index('Blade 1.03 Temp')]

      }
    switch_3901Rerr_dic={
      'Switch Status':['^(?!Active)',switch3901R_hdr.index('Switch Status')],
      'PSU1 Error':['^(?!(?:GOOD|NA))',switch3901R_hdr.index('PSU1 Status')],
      'PSU2 Error':['^(?!(?:GOOD|NA))',switch3901R_hdr.index('PSU2 Status')],
      'FAN Error':['^(?!(?:GOOD|NA))',switch3901R_hdr.index('FAN Status')],

      'Blade 1.01 Error':['^(?!(?:GOOD|NA))',switch3901R_hdr.index('Blade 1.01 Status')],

      'Blade 1.01 Temp':['^[5-9]* C',switch3901R_hdr.index('Blade 1.01 Temp')],

      'Blade 1.01 Conn Filter > 90%':['^9[1-9]|^100',switch3901R_hdr.index('Blade 1.01 Temp')]
    ...
    html+=get_error_output(switch_3903err_dic, switches_3903data)
    html+=get_error_output(switch_3901Rerr_dic, switches_3901Rdata)
    ...
    email_html = """
       ...
       <style>
         ...
         font-size:85%%
          ...
          <body>
          <h5>PFS Health-Check Scan v%s<br>%s<br></h5>""" %(time.ctime(),VERSION)
    email_html+= "<p>"
...
if __name__ == '__main__':
    ...
    print "   ( (__) )  "
    print "    \____/   AUTOMATED 3903 & 3901R PFS SERVER SCAN"
    print "             "
    #
