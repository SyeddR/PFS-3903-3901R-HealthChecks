...
FOLDER         = '/opt/scripts/PFS_script' ### Set the path to script's folder
Last_Scanpath  = FOLDER+'/PortScan_Results'
...
class Blade:
    ...
    def __init__(self,name,version=None,status=None):
...
def GetSwitch_Status(_p,switch):
  try:
        ...
        try:
           ...
           ## Switch status 
           switch._status =  re.findall('Switch Model.*Status: (\w*)',data)[0].rstrip()
       ...
       ## diagstat cha "cnt" 
        ...
        if i == 0:
           ...
           data1=p.before
           print data1
           switch._brdstat = re.findall('(Slot.*board.*PRESENT)',data1)
...
def Getbladestatus(p,blade):
  try :
       ...
       i = p.expect(["ERROR:", pexpect.TIMEOUT,pexpect.EOF, COMMAND_PROMPT], timeout=60)
       if i == 0:
           blade._Status = "Switch returned an error:" + p.after
           print "Switch returned an error:" + p.after
       elif i == 1:
           blade._Status = "Failed to get switches: Timeout"
           print "Failed to get switches: Timeout"
       elif i == 2:
    ...
    print "Cannot get Blad"+blade._Name+"status"      
...
#def BoardScan(server,path):
#
#    try:
#        for switch in server._Switches:
#            if os.path.isfile(path+'/'+switch._Name+'_board_scan'):
#                board_oldstatus=[]
#                f = open(path+'/'+switch._Name+'_board_scan', 'rb')
#                for line in f:
#                    #line=line.split()
#                    board_oldstatus.append(line)
#                for i in switch._ports:
#                    if i in port_oldstatus and port_oldstatus[i]!=switch._ports[i] and switch._ports[i] == 'Not-Connected':
#                         switch._portschng[i]=[port_oldstatus[i],switch._ports[i]]
#                f.close()
#            
#            f=open(path+'/'+switch._Name+'_port_scan', 'wb')
...
def usage():
    ...
    print "   Outputs: "
    print "   "
    print "       A log file for each server will be added to the directory with the servers ip address"
    print "   "
    print "       Each log file has the server's ip address, then the time when the log file was made"
    print "   "
    print "       Naming Convention: server_scan_overview_<timestamp>.txt"
    print "   "
    print "       A more detailed report for each switch is included in its own directory for each server."
    print "   "
...
def GetServerHealth (server):
    try:  
        ...
        output, err = ssh_command(server._Ip, omreport_command)
        ...
        output, err = ssh_command(server._Ip, hostname_command)
        ...
        server._hc=" Cannot get HC data of %s"%server._Ip 
...
def proto_main(argv):
    ...
    for server in servers: 
        try:
          data=server._Name+','+server._Ip+','+','.join(server._hc)
          file1.write(data)
          file1.write('\n')
          servers_data.append(data.split(','))
          
        except:
             print """file 1 cannot be written"""
    ...
    server_err_dic={}
    for item in server_hdr[2:]:
       server_err_dic[item+' Failure']=['^(?!Ok)',server_hdr.index(item)]
       
    
    switch_err_dic={
      'Switch Status':['^(?!Active)',switch_hdr.index('Switch Status')],
      'PSU1 Error':['^(?!GOOD)',switch_hdr.index('PSU1 Status')],
      'PSU2 Error':['^(?!GOOD)',switch_hdr.index('PSU2 Status')],
      'FAN Error':['^(?!GOOD)',switch_hdr.index('FAN Status')],
      #'Max Power Budget',
      #'Slot 1 Status',
      #'Slot 2 Status',
      #'Slot 3 Status',
    ...
    email_html = """
       ...
       <style>
         ...
         font-size:85%%
          ...
          <body>
          <h5>PFS Health-Check Scan v1.0<br>%s<br></h5>""" %time.ctime()
    email_html+= "<p>"
...
if __name__ == '__main__':
    ...
    print "   ( (__) )  "
    print "    \____/   AUTOMATED SERVER SCAN"
    print "             "
